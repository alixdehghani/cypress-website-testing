describe('Dashboard Features', () => {
    ...
    it('click query result', () => {
        cy.fixture('oss-config').then((ossConfig) => {
            ...
            cy.get(`mat-tree-node`, { timeout: 10000 }).contains("Uplink measurements").click();
            const selectedCounters = ossConfig["Uplink measurements"].counters.filter(item=> item.isActive).map(item => item.name);
            const selectedKpis = ossConfig["Uplink measurements"].kpis.filter(item=> item.isActive).map(item => item.name);
            ...
            allSelected.forEach(c => {
                ...
                const newName = `Uplink measurements-${c}-chart-${timestamp}.png`;
