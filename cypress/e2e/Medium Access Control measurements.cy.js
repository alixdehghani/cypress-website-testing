describe('Dashboard Features', () => {
    ...
    it('click query result', () => {
        cy.fixture('oss-config').then((ossConfig) => {
            ...
            cy.get(`mat-tree-node`, { timeout: 10000 }).contains("Medium Access Control measurements").click();
            const selectedCounters = ossConfig["Medium Access Control measurements"].counters.filter(item=> item.isActive).map(item => item.name);
            const selectedKpis = ossConfig["Medium Access Control measurements"].kpis.filter(item=> item.isActive).map(item => item.name);
            ...
            allSelected.forEach(c => {
                ...
                const newName = `Medium Access Control measurements-${c}-chart-${timestamp}.png`;
