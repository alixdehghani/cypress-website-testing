describe('Dashboard Features', () => {
    ...
    it('click query result', () => {
        cy.fixture('oss-config').then((ossConfig) => {
            ...
            const selectedCounters = ossConfig.counters["Enhanced Radio Access Bearer measurements"].filter(item=> item.isActive).map(item => item.counter_name);
            ...
            selectedCounters.forEach(c => {
                ...
                const newName = `Enhanced Radio Access Bearer measurements-${c}-chart-${timestamp}.png`;
