describe('Dashboard Features', () => {
    ...
    it('click query result', () => {
        cy.fixture('oss-config').then((ossConfig) => {
            ...
            cy.get(`mat-tree-node`, { timeout: 10000 }).contains("S1 measurements").click();
            const selectedCounters = ossConfig["S1 measurements"].counters.filter(item=> item.isActive).map(item => item.name);
            const selectedKpis = ossConfig["S1 measurements"].kpis.filter(item=> item.isActive).map(item => item.name);
