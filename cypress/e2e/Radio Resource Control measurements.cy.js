describe('Dashboard Features', () => {
    ...
    it('click query result', () => {
        cy.fixture('oss-config').then((ossConfig) => {
            ...
            cy.get(`mat-tree-node`, { timeout: 10000 }).contains("Radio Resource Control measurements").click();
            const selectedCounters = ossConfig["Radio Resource Control measurements"].counters.filter(item=> item.isActive).map(item => item.name);
            const selectedKpis = ossConfig["Radio Resource Control measurements"].kpis.filter(item=> item.isActive).map(item => item.name);
            ...
            allSelected.forEach(c => {
                ...
                const newName = `Radio Resource Control measurements-${c}-chart-${timestamp}.png`;
