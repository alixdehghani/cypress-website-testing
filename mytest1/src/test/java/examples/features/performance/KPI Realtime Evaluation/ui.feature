...
Scenario: login and see counter monitoring
  ...
  * def countersTab = "//a[text()='KPI Realtime Evaluation']"
  And waitFor(countersTab).click()

  And waitForUrl('/performance/evaluate-kpi') 
  ...
  * def startButton = "//button[.//mat-icon[text()='play_circle'] and contains(., 'Start')]"
