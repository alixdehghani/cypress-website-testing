...
Scenario: login with static param, navigate and create a new user
  ...
  * def softwareTab = "//a[.//mat-icon[text()='devices']]"
  And waitFor(softwareTab).click()
    * def terminalTab = "//a[text()='Terminal']"
    And waitFor(terminalTab).click()
    And waitForUrl('/software/terminal')
    
    And waitFor('textarea.xterm-helper-textarea')
  And click('textarea.xterm-helper-textarea')
  And input('textarea.xterm-helper-textarea', ' clear ')
  And delay(5000)
  And script("document.querySelector('textarea.xterm-helper-textarea').dispatchEvent(new KeyboardEvent('keydown', {key: 'Enter', keyCode: 13}))")
